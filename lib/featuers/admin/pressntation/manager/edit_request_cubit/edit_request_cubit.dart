...
class EditRequestCubit extends Cubit<EditRequestState> {
  ...
  Future<void> selectStartTime(
      BuildContext context,DateTime date,String hallId,String reservatoinId,String userId,String requestId,Timestamp initialStartTime) async {
    ...
    for (String path in pathsToUpdate) {
      await FirebaseFirestore.instance.doc(path).update({'startTime': pickedTimesTemp});
    }
  ...
  void selectEndTime(
      BuildContext context, DateTime selectedDate,String hallId,String reservatoinId,String userId,String requestId,Timestamp initialEndTime) async {
    ...
    for (var path in pathsToUpdate) {
      await FirebaseFirestore.instance.doc(path).update({'endTime': pickedTimesTemp});
